...
--merge un matching mai specific, cu Empty doar left, doar right, sau ambele

--extra (gpt)
balance :: IntSearchTree value -> IntSearchTree value
balance tree = buildBalancedTree (toList tree)

-- Functia care construieste un arbore echilibrat dintr-o lista ordonata
buildBalancedTree :: [(Int, value)] -> IntSearchTree value
buildBalancedTree [] = Empty
buildBalancedTree xs = 
  let middle = length xs `div` 2
      (left, (key, value):right) = splitAt middle xs
  in BNode (buildBalancedTree left) key (Just value) (buildBalancedTree right)
